...
export function PwaStatus() {
  ...
  if (online && pendingCount === 0 && !installPrompt) {
    return null;
  }

  return (
    <div className="fixed inset-x-3 bottom-[calc(5.6rem+env(safe-area-inset-bottom))] z-50 mx-auto max-w-xl lg:bottom-5 lg:left-auto lg:right-5 lg:mx-0 lg:w-96">
      <div className="rounded-[20px] border border-white/80 bg-white/95 p-3 shadow-2xl shadow-slate-900/15 backdrop-blur-xl">
        ...
        {installPrompt && (
          <button
            onClick={install}
            className="mt-2 flex min-h-11 w-full items-center justify-center gap-2 rounded-lg bg-slate-950 px-4 text-sm font-bold text-white first:mt-0"
          >
            <Download size={18} />
            Instalar Ovelhas
          </button>
